...
function SendScreen() {
   ...
   const CHUNK_SIZE = 1 * 1024 * 1024; // 1 MB chunks — fewer round-trips, less JSON overhead
   const WS_BUFFER_HIGH_WATER_MARK = 8 * 1024 * 1024; // 8 MB high-water mark — less frequent stalls
   ...
   const waitForSocketDrain = async (ws: WebSocket) => {
      if (ws.bufferedAmount <= WS_BUFFER_HIGH_WATER_MARK) return;
      await new Promise<void>((resolve) => {
         const check = () => {
            if (
               ...
               // Use requestAnimationFrame when available for tighter drain loop
   ...
   const streamSingleFile = async (queued: QueuedFile) => {
      ...
      while (offset < file.size) {
         ...
         chunkIndex++;
         sentBytesAccum += chunk.byteLength;

         // Throttle React state updates to ~10 fps to avoid render thrash
         const now = Date.now();
         if (isLastChunk || now - lastUiUpdate >= 100) {
            const captured = sentBytesAccum;
            setTransferSentBytes((prev) => prev + captured);
            sentBytesAccum = 0;
            lastUiUpdate = now;
         }
      }
   ...
   const shouldCreateArchive = (items: QueuedFile[]) => {
      return (
         items.length > 1 ||
         items.some((item) => item.relativePath.includes("/"))
      );
   };
   ...
   const startTransfer = async () => {
      ...
      try {
         ...
         setError(
            "Failed to prepare or send selected items. Please try again.",
         );
      } finally {
         setIsSending(false);
      }
      ...
      <>
         ...
         <div className="min-h-screen bg-gradient-to-b from-[#0E0E0E] via-[#151515] to-[#1a1a1a] text-[#D9D9D9] p-6 relative overflow-hidden">
            ...
            >
               ...
               >
                  Create a room and share the room ID with the recipient to
                  start sharing files.
               </motion.p>
                  ...
                  >
                        ...
                        >
                           ...
                           {isSending && (
                              <div className="text-sm text-[#FFD700] space-y-1">
                                 ...
                                 <p className="text-[#D9D9D9]/80">
                                    Speed: {formatTransferSpeed(senderSpeedBps)}{" "}
                                    • ETA: {formatEta(senderEtaSeconds)}
                                 </p>
                              </div>
                           )}
                        </motion.div>
                     ...
                     >
                        ...
                        <p className="text-left leading-relaxed">
                           ...
                           Peer-to-peer transfer! Data isn't stored on the
                           backend. Select files/folder first, then press Send.
                           Multiple files or folders are auto-zipped before
                           transfer.
                           {isSecureMode
