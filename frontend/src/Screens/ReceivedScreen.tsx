...
function ReceivedScreen() {
   ...
   const finalizeTransfer = async (mimeType: string) => {
      ...
      const completeFile = new Blob(memoryChunksRef.current, {
         type: mimeType,
      });
   ...
   const handleIncomingChunk = async (data: any) => {
      ...
      // Throttle progress UI updates to ~10 fps — avoids re-render storm on every chunk
      const now = Date.now();
      if (data.isLastChunk || now - lastUiUpdateRef.current >= 100) {
         const newProgress = ((data.chunkIndex + 1) / data.totalChunks) * 100;
         // Batch all three state updates into a single React render pass
         // by using a flushSync-free approach: update in sequence within same tick
         setReceivedBytes((prev) => prev + chunkArray.byteLength);
         setReceivedChunks(data.chunkIndex + 1);
         ...
         <div className="min-h-screen bg-gradient-to-b from-[#0E0E0E] to-[#1a1a1a] text-[#D9D9D9] p-6 relative overflow-hidden">
            ...
            >
               ...
               <p className="text-sm mt-2">
                  Files are transferred in chunk-streaming mode to minimize RAM
                  usage
               </p>
            </motion.div>
         </div>
